...
def scout_report(data_frame, gender, league, season, xtra, template, pos, player_pos, mins, minplay, compares, name, ws_name, team, age, sig, extra_text, custom_radar, dist_labels, logo_dict, metric_selections=None):
    ...
    if custom_radar == 'y':
        ...
        names_dict = dict(zip(all_possible_vars, names))
        base_var_names_dict = dict(zip(all_possible_vars, base_var_names))
        
        ix_selected = []
        ix_selected = [v for v in all_possible_vars if v[0] in metric_selections]
        # for i, l in enumerate(all_possible_vars):
        #     if l in metric_selections:
        #         ix_selected+=[i]
        metric_rename = []
        for ix in ix_selected:
            metric_rename+=[names_dict[ix]]
        base_vars = []
        for ix in ix_selected:
            base_vars+=[base_var_names_dict[ix]]
...
with st.expander('Instructions'):
    st.write('''
    VIDEO TUTORIAL: https://www.youtube.com/watch?v=s3Ai97SSARw  \n
    This app is a tool to help you find players who might be performing well in specific roles, lets you generate a player's performance radar so that you can see how their data stacks up against others in their position, offers filtering to aid player discovery, and lets you make scatter plots to quickly compare players in 3 variables.  \n
...
if gender == 'Men':
    df = pd.read_csv(f'https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/main/Main%20App/{full_league_name.replace(" ","%20").replace("ü","u").replace("ó","o").replace("ö","o").replace("ã","a").replace("ç","c").replace("é","e")}.csv')
elif gender == 'Women':
    df = pd.read_csv(f'https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/main/Main%20App/Women/{full_league_name.replace(" ","%20").replace("ü","u").replace("ó","o").replace("ö","o").replace("ã","a").replace("ç","c").replace("é","e")}.csv')
...
with radar_tab:
    ...
    #         metric_selections = sorted(metric_selections)
    #     else:
    #         custom_radar_q = 'n'
