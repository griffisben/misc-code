...
alt.themes.register('own_theme', own_theme)
alt.themes.enable('own_theme')
...
raw_df = pd.read_csv(f'https://raw.githubusercontent.com/griffisben/misc-code/files/{formatted_name}%20API%20RAW%20DATA.csv')
...
pos_select = alt.selection_point(fields=['main_position'], bind=pos_dropdown, clear='dblclick')

teams = ['']+sorted(data.club.unique().tolist())
team_dropdown = alt.binding_select(options=teams, name="Team")
team_select = alt.selection_point(fields=['club'], bind=team_dropdown, clear='dblclick')
...
points = base.mark_bar(
    ...
    strokeWidth=.5, strokeOpacity=.8
).encode(
    x=alt.X(f'mean({var}/90):Q', title=f"{var.replace('_',' ').title()} per 90'").scale(domain=(0, x_domain_max)),
    y=alt.Y('player', title="Player", sort=alt.EncodingSortField(f'{var}/90', op="mean", order='descending')),
    color=alt.condition(selector, alt.value('#806c5e'), alt.value('lightgray'), legend=None),
    tooltip=[
        "player",
        "age",
        "main_position",
    ...
    title=alt.Title(
        text=f"{league} {ssn}, {var.replace('_',' ').title()} per 90'",
        subtitle=f"Includes players with at least {mins} minutes played | Data via Wyscout, created by Ben Griffis (@BeGriffis)",
        subtitleFontSize=12,
        anchor='middle'
    )
)

timeseries = base.mark_bar(width=8, strokeOpacity=.8).encode(
    x=alt.X('date:T', title='Match Date').scale(domain=(date_min,date_max)),
    y=alt.Y(f"{var}/90", title=f"{var.replace('_',' ').title()} per 90'", stack=None),
    color=alt.Color('minutes_on_field', title='Minutes Played'),
    tooltip=[
        "match_name",
        'date',
        "player",
    ...
    title=alt.Title(
        text=f"{var.replace('_',' ').title()} per 90', Match-by-Match Breakdown",
        subtitle=f"Only includes games that players played at least {min_mins} mins.",
        subtitleFontSize=12,
        anchor='middle'
    )
)
