...
def scout_report(data_frame, gender, league, season, xtra, template, pos, player_pos, pos_in_sample, mins,maximum_mins, minplay, compares, name, ws_name, team, age, sig, extra_text, custom_radar, dist_labels, logo_dict, metric_selections=None):
    ...
    if custom_radar == 'n':
        ...
        if template == 'strikers':
            raw_vals = raw_valsdf[["Player",
                                extra12,extra14,extra13,fwd7 ,fwd4 ,fwd3 ,extra17,extra6 ,fwd9 ,extra16,fwd5 ,extra7 ,fwd12 ,extra18,def3 ,mid12 ,extra2 ,fwd2 ,extra9 ,fwd1 ,extra19,fwd6 ,fwd11,extra8
                              ]]
        ...
        if template == 'centerbacks':
            raw_vals = raw_valsdf[["Player",
                                gk8 ,gk11 ,mid2, extra20, extra13 ,extra11,extra14,fwd4 ,def10 ,fwd9 ,extra16,extra7 ,fwd12 ,extra18,def3 ,mid12 ,def4 ,extra2 ,fwd2 ,fwd11
                              ]]
            raw_vals_full = raw_valsdf_full[["Player",
                                gk8 ,gk11 ,mid2, extra20, extra13 ,extra11,extra14,fwd4 ,def10 ,fwd9 ,extra16,extra7 ,fwd12 ,extra18,def3 ,mid12 ,def4 ,extra2 ,fwd2 ,fwd11
                              ]]
    ...
    if custom_radar == 'y':
        all_possible_vars = [
            'Received passes per 90', 'Passes per 90', 'Pct of passes being short',
            'Pct of passes being lateral', 'Accurate passes, %', 'Accurate short / medium passes, %',
            'Accurate long passes, %', 'Crosses per 90', 'Accurate crosses, %', 'Smart passes per 90',
            'Shot assists per 90', 'xA per 90', 'xA per Shot Assist', 'Assists per 90',
            'Second assists per 90', 'Third assists per 90', '1st, 2nd, 3rd assists',
            'Progressive passes per 90', 'Progressive runs per 90', 'Shots per 90', 'npxG per 90',
            'Non-penalty goals per 90', 'npxG per shot', 'Goal conversion, %', 'Successful dribbles, %',
            'Accelerations per 90', 'Touches in box per 90', 'Fouls suffered per 90',
            'Successful defensive actions per 90', 'Duels won, %', 'Defensive duels won, %',
            'pAdj Tkl+Int per 90', 'PAdj Sliding tackles', 'PAdj Interceptions', 'Shots blocked per 90',
            'Aerial duels per 90', 'Aerial duels won, %', 'Aerial duels won per 90', 'Fouls per 90',
            'Shots against per 90', 'Conceded goals per 90', 'Save rate, %', 'Prevented goals per 90',
            'Goals prevented %', 'Clean sheets, %', 'Exits per 90', 'Average long pass length, m'
        ]
        names = [
        ...
        base_var_names = [
            'gkpct13', 'gkpct8', 'gkpct11', 'gkpct12', 'extrapct', 'midpct1', 'midpct2', 'extrapct10',
            'extrapct3', 'extrapct4', 'midpct4', 'midpct5', 'extrapct5', 'midpct6', 'midpct7', 'midpct8',
            'defpct10', 'midpct9', 'midpct10', 'extrapct2', 'fwdpct2', 'fwdpct1', 'extrapct9', 'fwdpct6',
            'fwdpct5', 'extrapct6', 'fwdpct11', 'extrapct8', 'defpct1', 'midpct11', 'defpct3', 'midpct12',
    ...
    if dist_labels == 'No':
        dist_text = ""
    if maximum_mins == 5000:
        min_max_mins_text = f"{mins}+"
    else:
        min_max_mins_text = f"{mins}-{maximum_mins}"
    plt.annotate(f"Bars are percentiles | Values shown are {callout_text} values\nAll values are per 90 minutes | %s\nCompared to %s %s, {min_max_mins_text} mins\nData: Wyscout | Sample Size: %i players{dist_text}" %(extra_text, league, compares, len(dfProspect)),
...
if gender == 'Women':
    ...
    default_danish_league = "NWSL"
    default_danish_season = "2025"
    logo_dict_df = read_csv('https://raw.githubusercontent.com/griffisben/Wyscout_Prospect_Research/main/Wyscout_Logo_Dict_Women.csv')
    logo_dict = pd.Series(logo_dict_df['Team logo'].values,index=logo_dict_df['Team']).to_dict()
...
with st.sidebar:
    ...
    maximum_mins = st.number_input('Maximum Minutes Played', 500, 5000, 5000)
...
def make_fig(ages,exp_contracts,rank_11_base,show_ranks2,season,lg,normalize_to_100,team_text,chosen_nations):
    ...
    axs['title'].text(0.5, 1.35, f'Data via Wyscout | {lg_lookup[(lg_lookup.League==lg) & (lg_lookup.Season==season)].Date.values[0]} | Created on best11scouting.streamlit.app',
...
with filter_tab:
    ...
    if geo_input_filters == 'League':
        with st.form('Time Frame Filters, League'):
            ...
            time_frame_filters = st.multiselect('League-Seasons', (similar_player_lg_lookup_filters[similar_player_lg_lookup_filters.League.isin(region_filters)].sort_values(by=['Country','Tier','Season'],ascending=[True,True,False])['League-Season'].tolist()), default=similar_player_lg_lookup_filters[similar_player_lg_lookup_filters.League.isin(region_filters)].sort_values(by=['Country','Tier','Season'],ascending=[True,True,False])['League-Season'].tolist()[0])
...
with filter_tab:
    ...
    with st.form('Minimum Percentile Filters'):
        ...
        ast123 = st.slider('1st, 2nd, & 3rd Assists per 90', 0.0, 1.0, 0.0, key='slider12')
        ...
        num_dribs = st.slider('Successful dribbles per 90', 0.0, 1.0, 0.0, key='slider20')
        off_duels = st.slider('Offensive duels per 90', 0.0, 1.0, 0.0, key='slider21')
        off_win_pct = st.slider('Offensive Duel Win %', 0.0, 1.0, 0.0, key='slider22')
        accel = st.slider('Accelerations per 90', 0.0, 1.0, 0.0, key='slider23')
        progcarry = st.slider('Progressive Carries per 90', 0.0, 1.0, 0.0, key='slider24')
        progpass = st.slider('Progressive Passes per 90', 0.0, 1.0, 0.0, key='slider25')
        aerial = st.slider('Aerial Win %', 0.0, 1.0, 0.0, key='slider26')
        aerialswon = st.slider('Aerials Won per 90', 0.0, 1.0, 0.0, key='slider27')
        defduels = st.slider('Defensive Duels Success %', 0.0, 1.0, 0.0, key='slider28')
        defend = st.slider('Successful Defensive Actions per 90', 0.0, 1.0, 0.0, key='slider29')
        tklint = st.slider('Tackles & Interceptions per 90', 0.0, 1.0, 0.0, key='slider30')
        tkl = st.slider('Sliding Tackles per 90', 0.0, 1.0, 0.0, key='slider31')
        intercept = st.slider('Interceptions per 90', 0.0, 1.0, 0.0, key='slider32')
        shotblock = st.slider('Shots Blocked per 90', 0.0, 1.0, 0.0, key='slider33')
        foul = st.slider('Fouls Committed per 90', 0.0, 1.0, 0.0, key='slider34')
        fouldraw = st.slider('Fouls Drawn per 90', 0.0, 1.0, 0.0, key='slider35')
        cards = st.slider('Cards per 90', 0.0, 1.0, 0.0, key='slider36')
        if pos_select_filters == 'Goalkeepers':
            concede = st.slider('Conceded goals per 90', 0.0, 1.0, 0.0, key='slider37')
            saverate = st.slider('Save rate, %', 0.0, 1.0, 0.0, key='slider38')
            exits = st.slider('Exits per 90', 0.0, 1.0, 0.0, key='slider39')
            psxg = st.slider('Prevented goals per 90', 0.0, 1.0, 0.0, key='slider40')
            gppct = st.slider('Goals prevented %', 0.0, 1.0, 0.0, key='slider41')
            shotsfaced = st.slider('Shots against per 90', 0.0, 1.0, 0.0, key='slider42')
...
with scatter_tab:
    ...
    fig_scatter = px.scatter(
        ...
        hover_name = 'Player',
        title = '%s %s, %s & %s <br><sup>%s%s %i-%i minutes played | %s</sup><br><sup>Created on Best11Scouting.streamlit.app</sup>' %(season,lg,xx,yy,age_text_scatter,pos_select_scatter,mins,maximum_mins,update_date),
        width=900,
        height=700)
