...
with st.expander('Instructions'):
    ...
    3a) Pre-Made vs Custom Radar - the default setting here is the pre-made radar, which is a general template based on a player's position (attacking, defensive, CB, GK). However, you can also select your own metrics to include in the radar instead! First, select "Custom", and then click Submit. Then you can choose what metrics you want to include in your custom radar. Click submit again to generate the radar with your selected metrics. Every once in a while, Streamlit will whack out and if there's no image generating, make sure to check your player name & age again, sometimes the age changes randomly, sometimes the name disappears. When in doubt, clikc "Run" at the top or "Submit Options".  \n
